...
# anova_mse: the MSE that corresponds to anova_dat
# Tukey-HSD adjusted p-values for the 10 contrasts of the 4538 peptides at 5% BH FDR
# Tukey-HSD difference estimate
# Tukey contrasts confidence interval
...
For the 4538 peptides at 5\% BH FDR, we proceed to perform the Tukey HSD analysis. Basically, it is very similar to a pairwise t-test (except that it corrects for family-wise error rate) for every possible contrast (pairwise comparison between 2 groups of patients):
...
tukey_effect_estimate <- round(tukey_effect_estimate, 4)
tukey_adjusted_pval <- round(tukey_adjusted_pval, 4)
...
boxplot_func <- function(mat, col = boxplot_col, draw){
  ...
  abline( h=2.5, col="gray" )
  abline( h=4.5, col="gray" )
...
peptide_counts_df <- data.frame(
  ...
  Significant_peptide_counts = unname( apply(tukey_adjusted_pval,2, function(x){length(x[x <= tukey_cutoff])}) ) 
  ) 
ggplot(data=peptide_counts_df, aes(x=contrasts, y=Significant_peptide_counts)) + 
  geom_bar(stat="identity", width=0.5 , fill="steelblue") + 
  coord_flip() +
  geom_text(aes(label=Significant_peptide_counts), hjust=1.1, color = "white", size=4) +
  theme_minimal() 
```

Note that these are peptides that are identified to be significantly different between the pairwise contrasts comparison, but these pairwise differences could be either positive or negative. In general, we are more interested in the development of new/heightened antibody responses (in response to more severe form of cancer) rather than the ones that went away. We could order the level of cancer stages according to their severity (recall that ``mCS`` patients do not have technical replicates and are excluded in this analysis):  

``normal``, ``new_dx``, ``nmCS``, ``nmCR``, ``mCR``,  

and focus only on significant contrasts that are also positive in value, which indicates increased antibody responses in more severe form of prostate cancer. Now, we plot the number of peptides that have positive significant contrasts:

```{r, fig.height = 3.8}
positive_peptide_counts_df <- data.frame(
  contrasts = names(apply(significant_positive,2, function(x){length(x[x == 1])})),
  Positive_significant_peptide_counts = unname( apply(significant_positive,2, function(x){length(x[x == 1])}) ) 
  ) 
ggplot(data=positive_peptide_counts_df, aes(x=contrasts, y=Positive_significant_peptide_counts)) + 
  geom_bar(stat="identity", width=0.5 , fill="steelblue") + 
  coord_flip() +
  geom_text(aes(label=Positive_significant_peptide_counts), hjust = -0.05, color = "black", size=2) +
  theme_minimal() + 
  theme(axis.text=element_text(size=6), axis.title=element_text(size = 6))
```

We could then investigate peptides deemed significant under some interesting (positive) contrasts. We will focus on two of them:  

* ``mCR`` patients vs the others  
* ``normal`` subjects vs cancer patients  

## ``mCRPC`` vs the other stages

For instance, we are interested in peptides that exhibit **significantly higher** $\log_2$ fluorescence level in ``mCR`` patients compared to the rest of the patients. 
...
We tabulate all these peptides.  

```{r}
mCR_df <- data.frame(
  Peptide_ID = row.names(anova_dat)[mCR_ind],
  BH_FDR = unname(round( one_way_anova_BH[one_way_anova_BH <= 0.05][mCR_ind], 4 )),
  mCRPC_normal_diff = unname(tukey_effect_estimate[,'mCRPC-normal'][mCR_ind]),
  mCRPC_new_dx_diff = unname(tukey_effect_estimate[,'mCRPC-new_dx'][mCR_ind]),
  mCRPC_nmCSPC_diff = unname(tukey_effect_estimate[,'mCRPC-nmCSPC'][mCR_ind]),
  mCRPC_nmCRPC_diff = unname(tukey_effect_estimate[,'mCRPC-nmCRPC'][mCR_ind])
)
mCR_df <- mCR_df[order(mCR_df$BH_FDR),]

kable(mCR_df, format = "latex", row.names= F, col.names = NA, booktabs = T )%>%
  kable_styling(latex_options = c("scale_down", "hold_position"))
```

In addition, we also plot the boxplots of the fluorescence levels of the different groups of patients for some of these peptides. The x-axis of the boxplots refers to the $\log_2$ median fluorescence level of the patients. 

```{r, fig.height = 2.7}
boxplot_mat <- anova_dat[match(mCR_df$Peptide_ID[c(1,2,4)], row.names(anova_dat)),]
for (i in 1:nrow(boxplot_mat)){
  boxplot_func(mat = boxplot_mat, draw = i)
}
```

As an examnple, we also plot an error-bar figure for the contrasts of one of the significant peptides. 

```{r}
error_bar_ind <- match(mCR_df$Peptide_ID[1], row.names(anova_dat))
error_bar_df <- data.frame(
  contrasts = colnames(tukey_effect_estimate),
  difference = unname(tukey_effect_estimate[error_bar_ind,]),
  lower_CI = unname(tukey_CI_lower[error_bar_ind,]),
  upper_CI = unname(tukey_CI_upper[error_bar_ind,])
)

ggplot(error_bar_df, aes(x = contrasts, y = difference)) + 
  geom_point() +
  geom_errorbar(aes(ymin = lower_CI, ymax = upper_CI), width=.2, position=position_dodge(0.05)) +
  coord_flip() +
  geom_hline(yintercept = 0, col = "red", lty = 2) +
  labs(title = mCR_df$Peptide_ID[1]) + 
  theme(plot.title = element_text(hjust = 0.5))
```

## Healthy subjects vs cancer patients

For example, we could find peptides that are significantly different (could be higher or lower antibody responses) between healthy subjects and cancer patients (but not siginificant among the different stages of patients).

```{r, echo = T}
row.names(tukey_pairwise_pattern)[ which(
...
However, there are no peptides (among the 4538 peptides at 5\% BH FDR) that uniformly exhibit significantly higher fluorescence levels across all cancer patients compared to healthy subjects. 
...
This could be due to the small number of peptides (55 peptides) that have significantly higher antibody responses in ``mCRPC`` patients vs healthy subjects. Perhaps this also corroborates with the clustering patterns we observe in PCA, t-SNE and heatmap plots, where healthy subjects are "scattered all over the plots".  

Nevertheless, there are 2 peptides that exhibit significantly higher fluorescence levels in ``new_dx``, ``nmCS`` and ``nmCR`` patients compared to heathy subjects. We plot their error bars.
...
error_bar_df <- data.frame(
  contrasts = colnames(tukey_effect_estimate),
  difference = unname(tukey_effect_estimate[healthy_ind[1],]),
  lower_CI = unname(tukey_CI_lower[healthy_ind[1],]),
  upper_CI = unname(tukey_CI_upper[healthy_ind[1],])
)
...
ggplot(error_bar_df, aes(x = contrasts, y = difference)) + 
  ...
  coord_flip() +
  geom_hline(yintercept = 0, col = "red", lty = 2) +
  labs(title = row.names(tukey_effect_estimate)[healthy_ind[1]]) + 
  theme(plot.title = element_text(hjust = 0.5))
```
...
error_bar_df <- data.frame(
  contrasts = colnames(tukey_effect_estimate),
  difference = unname(tukey_effect_estimate[healthy_ind[2],]),
  lower_CI = unname(tukey_CI_lower[healthy_ind[2],]),
  upper_CI = unname(tukey_CI_upper[healthy_ind[2],])
)
...
ggplot(error_bar_df, aes(x = contrasts, y = difference)) + 
  ...
  coord_flip() +
  geom_hline(yintercept = 0, col = "red", lty = 2) +
  labs(title = row.names(tukey_effect_estimate)[healthy_ind[2]]) + 
  theme(plot.title = element_text(hjust = 0.5))
```
