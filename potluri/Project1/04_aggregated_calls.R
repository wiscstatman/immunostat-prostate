...
# we remove their earlier-stage records to ensure unique patients
...
sample_key = array_id_key %>%
  ...
  tally() %>%
  select(-n) 
sample_key$stage <- relevel(sample_key$stage, ref = "normal")
...
# keep only patients that appear in sample_key
calls <- calls %>% select(container:PROBE_ID, any_of(sample_key$id))

# remove peptides that have zero calls in ALL subjects
calls <- calls[ apply( calls %>% select(any_of(sample_key$id)) , 1, function(x){ !all(x==0) } ) , ]

# check
calls %>% select(any_of(sample_key$id)) %>% rowSums() %>% summary()

# make sure sample_key align with calls
calls_iii <- match( colnames( calls %>% select(any_of(sample_key$id)) ), sample_key$id )
sample_key <- sample_key[calls_iii , ]
sum(as.numeric( colnames( calls %>% select(any_of(sample_key$id)) ) == sample_key$id )) == nrow(sample_key)
...
for(i in 1:nrow(calls)){
  ...
  fit1 <- glm(y ~ stage, family = binomial(link = "logit"), data = sample_key)
...
# remove peptides that do not have any calls from raw_data_complete
raw_data <- raw_data[ raw_data$PROBE_ID %in% calls$PROBE_ID , ]
...
####################################################################################### 
#                       Another test -- one-way ANOVA based on                        #
#             peptides that have at least one call among all subjects                 #
####################################################################################### 

# get stage
# remember stage in sample_key not necessarily aligned to raw_data
stage_iii <- match( colnames(select( raw_data_median, any_of(array_id_key$id) )), sample_key$id )
stage <- sample_key$stage[stage_iii]
sum(as.numeric( colnames(select( raw_data_median, any_of(array_id_key$id) )) == sample_key$id[stage_iii] )) == nrow(sample_key)

# initiate
anova_pval <- rep(NA, nrow(raw_data_median))
names(anova_pval) <- raw_data_median$PROBE_ID
ncol_dat <- ncol(raw_data_median)
n <- nrow(sample_key)

# compute one-way anova p-values
for(i in 1:nrow(raw_data_median)){
  anova_pval[i] <- unname( unlist(summary( aov( 
    as.numeric(raw_data_median[i, (ncol_dat - n + 1) : ncol_dat]) ~ stage ) ))["Pr(>F)1"] )
  print(i)
}
...
# plot histogram of p-values
all_peptide_hist <- hist(anova_pval, breaks = 70, freq = F, 
...
# peptide counts at various FDR thresholds
count.func(anova_BH, seq(0.01, 0.1, by = 0.01))
count.func(anova_qval, seq(0.01, 0.1, by = 0.01))
...
BH_FDR_cutoff <- 0.05
anova_dat <- raw_data_median %>% dplyr::select(PROBE_ID, any_of(sample_key$id)) %>% filter((anova_BH <= BH_FDR_cutoff))

anova_dat_demean <- sweep(as.matrix(anova_dat %>% dplyr::select(-PROBE_ID)), 1, 
                          rowMeans(as.matrix(anova_dat %>% dplyr::select(-PROBE_ID))), "-") # centering by row
...
BH_FDR_normal <- apply(as.matrix(anova_dat %>% dplyr::select(-PROBE_ID)), 1, 
                       function(x){mean(x[as.character(stage) == "normal"])})
BH_FDR_cancer <- apply(as.matrix(anova_dat %>% dplyr::select(-PROBE_ID)), 1, 
                       function(x){mean(x[as.character(stage) != "normal"])})
BH_FDR_MSE <- apply(as.matrix(anova_dat %>% dplyr::select(-PROBE_ID)), 1, function(x){
  fit1 <- lm(x ~ stage)
  return(deviance(fit1)/df.residual(fit1))
})

# get linear contrast p-values
normal_counts <- length(stage[stage == "normal"])
cancer_counts <- length(stage[stage != "normal"])
BH_FDR_tstat <- (BH_FDR_cancer - BH_FDR_normal) / sqrt(BH_FDR_MSE * (1/cancer_counts + 1/normal_counts))
BH_FDR_tstat_pval <- 1 - pt( BH_FDR_tstat, df = (length(unique(sample_key$stage)) - 1) )
...
boxplot_mat <- as.matrix(boxplot_mat %>% dplyr::select(-PROBE_ID))
