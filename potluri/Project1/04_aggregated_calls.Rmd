...
We acknowledge the consensus that the binary calls on a peptide of a patient could be conservative -- out of 177,604 peptides, only `r nrow(calls)` of them have at least one call among all patients. Most of the peptides have very few positive calls among all patients. We plot the distribution of the sum of calls among all patients for each peptide.
...
We could lose a lot of information due to the conservative signal threshold of the calls data. Instead, what if we use the calls data as a filtering criterion. Specifically, we first compute the median of the $\log_2$ fluorescence levels of each patient's replicates, and then subset the `r nrow(calls)` peptides have at least one call among all patients. For each of these `r nrow(calls)` peptides, we perform one-way ANOVA (analysis of variance) to test the following:  
...
After getting p-values for the `r nrow(calls)`  peptides, we plot the p-value histogram.
...
```{r}
kable( count.func(calls_anova$anova_BH, seq(0.01, 0.1, by = 0.01)),
       format = "latex", row.names= NA, col.names = NA, booktabs = T )%>%
  kable_styling(latex_options = c("scale_down", "hold_position")) %>%
  row_spec(1, hline_after = T)
```

We also tabulate the peptide counts at various q-value thresholds as shown below.

```{r}
kable( count.func(calls_anova$anova_qval, seq(0.01, 0.1, by = 0.01)),
       format = "latex", row.names= NA, col.names = NA, booktabs = T )%>%
  kable_styling(latex_options = c("scale_down", "hold_position")) %>%
  row_spec(1, hline_after = T)
```
...
We have identified `r length(calls_anova$anova_BH[calls_anova$anova_BH <= 0.05])` peptides at 5\% Benjamini-Hochberg(BH) false discovery rate (FDR). We would like to obtain some graphical representations to illustrate how the $\log_2$ fluorescence levels differ across different cancer stages for these `r length(calls_anova$anova_BH[calls_anova$anova_BH <= 0.05])` peptides.  
...
* The mCRPC points are clustered together near top-right of the plot.
* Most of the nmCRPC patients are not too far off from the mCRPC subjects in the middle of the plot.
...
Finally, we plot the heatmap of the $\log_2$ fluorescence (after removing row means) of the `r length(calls_anova$anova_BH[calls_anova$anova_BH <= 0.05])` peptides at 5\% BH-FDR.
...
Specifically, it is a one-sided t-test comparing the averages of the $\log_2$ fluorescence levels among cancer patients and among healthy subjects, except that now, the degrees of freedom is based on the one-way ANOVA in the previous step, and the standard error of the t-test statistic is based on the root mean-squared error obtained from the one-way ANOVA and adjusted for the number of the healthy and cancer patients. This allows us to pull samples from different cancer stages and increase power -- with the caveat/assumption that the patients of different cancer stages share the same level of variation in antibody response. (Note that we could avoid doing the multiplicity control like Tukey's Honest Significant Difference if we are only interested in a particular contrast/comparison and not every possible combination of pairwise comparisons)
...
Among the `r length(calls_anova$anova_BH[calls_anova$anova_BH <= 0.05])` peptides at 5\% BH-FDR, only `r length(BH_FDR_tstat_pval[BH_FDR_tstat_pval <= 0.05])` of them are associated with higher antibody response in cancer patients than in normal subjects. We tabulate these peptides with their differences (of average $\log_2$ fluorescence levels between cancer and healthy subjects) and p-values of their t-statistics. 
...
kable( contr_df, format = "latex", row.names= F, booktabs = T )%>%
  kable_styling(font_size = 8, latex_options = c("hold_position")) 
```

For purpose of visualization, we also produce the boxplots of $\log_2$ fluorescence levels of subjects from these two groups for some of these peptides.
...
# Tests on Fluorescence Levels without Filtering

As a comparison, we repeat the same calculation in the previous section, except that now, we do not apply any filtering on the peptides at all. First, we plot the histogram of p-values from the ANOVA.
...
We also tabulate the peptide counts at different BH FDR thresholds.
...
It appears that we have identify `r length(all_anova$anova_BH[all_anova$anova_BH <= 0.05])` significant peptides at 5\% BH FDR. Out of these `r length(all_anova$anova_BH[all_anova$anova_BH <= 0.05])` significant peptides, `r sum(as.numeric(names(all_anova$anova_BH[all_anova$anova_BH<=0.05]) %in% names(calls_rowsums[calls_rowsums >=1])))` of them (or `r round( sum(as.numeric(names(all_anova$anova_BH[all_anova$anova_BH<=0.05]) %in% names(calls_rowsums[calls_rowsums >=1]))) / length(all_anova$anova_BH[all_anova$anova_BH <= 0.05]) * 100 ,4)` \% of them) also have at least one call among all patients. Now, plot the resulting PCA, t-SNE and heatmap of the residual $\log_2$ fluorescence of these peptides.
...
We observe similar patterns in the PCA plot as we did when we apply the calls-filtering. Obviously, now that we have working with more peptides, the percentage of variance explained in the first principal component is slightly lower than what we achieved when we apply the calls-filtering.  
...
Again, we observe similar patterns in the t-SNE plot as we did when we apply the calls-filtering. There seems to be 2 clusters of nmCRPC patients.  
...
Again, we observe similar clustering patterns in the heatmap as we did when we apply the calls-filtering. 

## Comparing Healthy Subjects vs Cancer Patients

Finally, we repeat the linear contrast method on the `r length(all_anova$anova_BH[all_anova$anova_BH <= 0.05])` peptides at 5\% BH FDR to determine if fluorescence levels of these peptides are significantly higher among cancer patients than among healthy subjects.
...
* There appears to be cancer stage effects present in the $\log_2$ fluorescence data with or without filtering peptides based on calls. The clustering patterns of data points are similar with or without calls-filtering.  
