...
# Visualization of Treatment Effects (Higher Antibody Response in PAP)
...
signif_crit_sum_PAP <- sum(as.numeric(signif_crit))
...
It appears that treatment effects are prominent among many peptides. For the purpose of visualizing more prominent treatment effects brought about by PAP vaccine as compared to ADT, we will focus on the significant peptides at 1 \% FDR and that the PAP vaccine leads to higher antibody response by at least two-fold as compared to ADT at times 3-months and 6-months, ie. $\left( \widehat{\beta}_2 + 3 \times \widehat{\beta}_3 \right) \geq 1$ and $\left( \widehat{\beta}_2 + 6 \times \widehat{\beta}_3 \right) \geq 1$. There are `r signif_crit_sum_PAP` peptides that fulfill these criteria. We exported the list of these peptides to the Sheet "*Higher_in_PAP*" in the Excel file *05_Longitudinal_SignifPeptides.xlsx*. 
...
legend('topright', cex = 0.5, pch = shp_proj2[c('ADT_3','PAP_3')], col = c("blue","red"), c('ADT_3','PAP_3') )
PCload.func(cols_time3, shapes_time3, U, D, 3, 2, pca.var, title = "PC2 vs PC3") # PC loadings (PC2 vs PC3)
```

```{r, fig.height=6, cache = T, dependson=c("anova_dat_demean3","shp_proj2")}
...
```{r, fig.height=6, cache = T, dependson=c("anova_dat_demean6", "shp_proj2")}
...
We also show the boxplots of fluorescence levels for a few significant peptides that have higher antibody response in PAP compared to ADT
...
# Visualization of Treatment Effects (Higher Antibody Response in ADT)

```{r}
BH_FDR_cutoff <- 0.01
signif_crit <- signif_crit_func(-1)
signif_crit_sum_ADT <- sum(as.numeric(signif_crit))
...
We would also like to find out about the peptides for which ADT causes higher fluorescence levels as compared to the PAP vaccine. This time, we will focus on the significant peptides at 1 \% FDR and that the ADT leads to higher antibody response by at least two-fold as compared to PAP at times 3-months and 6-months, ie. $\left( \widehat{\beta}_2 + 3 \times \widehat{\beta}_3 \right) \leq -1$ and $\left( \widehat{\beta}_2 + 6 \times \widehat{\beta}_3 \right) \leq -1$. There are `r signif_crit_sum_ADT` peptides that fulfill these criteria. We exported the list of these peptides to the Sheet "*Higher_in_ADT*" in the Excel file *05_Longitudinal_SignifPeptides.xlsx*.  

We obtain the residuals of the null model at times 3-months and 6-months, and plot these residuals using the PCA, t-SNE and heatmap approaches. Any observed clustering patterns based on treatment types would reveal the treatment effects not captured in the null model. The clustering patterns observed here are less obvious than the ones we observe for the peptides that exhibit higher antibody responses in PAP.
...
par(mfrow = c(1,2), pty = "s", mar = c(2.2,2.3,1.5,0.45), mgp = c(1.4,0.4,0),
    cex.axis = 0.84, cex.lab = 0.84, cex.main = 0.84, tcl = -0.4)
PCload.func(cols_time3, shapes_time3, U, D, 1, 2, pca.var, title = "PC2 vs PC1") # PC loadings (PC2 vs PC1)
legend('bottomright', cex = 0.5, pch = shp_proj2[c('ADT_3','PAP_3')], col = c("blue","red"), c('ADT_3','PAP_3') )
PCload.func(cols_time3, shapes_time3, U, D, 3, 2, pca.var, title = "PC2 vs PC3") # PC loadings (PC2 vs PC3)
```
...
We also show the boxplots of fluorescence levels for a few significant peptides that have higher antibody response in ADT compared to PAP.

```{r, fig.height = 11, fig.width=8.5}
BH_FDR_cutoff <- 0.0005
signif_crit_func <- function(Eff_thresh){
  if (Eff_thresh > 0){
    signif_crit <- (KR_LMM$LMM_BH <= BH_FDR_cutoff) & 
      (Satterthwaite_LMM$LMM_BH <= BH_FDR_cutoff) &
...
* Out of the `r fdr1_count` peptides at 1\% FDR, `r signif_crit_sum_PAP` of them also exhibit higher fluorescence levels by at least two-fold due to the PAP vaccine effect as compared to ADT at both times 3-months and 6-months.  

* These `r signif_crit_sum_PAP` peptides demonstrate more obvious clustering effect (based on treatment types) in the PCA, t-SNE and heatmap at 6 month's time rather than at 3 month's time. This also suggests presence of *time* fixed effect in the data set.  

* Meanwhile, out of the `r fdr1_count` peptides at 1\% FDR, `r signif_crit_sum_ADT` of them also exhibit higher fluorescence levels by at least two-fold due to ADT as compared to PAP at both times 3-months and 6-months.  
