...
Note that these are not distinct patient counts, because there were 11 patients who were measured at two different stages. Number of replicates for each patient, ``rep`` could 1, 2, or 3. We keep patients with ``rep`` = 1. He also kept the latest distinct patient records only, so the distinct patient counts are:  
...
Hemanth has already checked that replicates for each patient largely "agree with one another". We take $\log_2$ transformation of the fluorescence intensity and compute the median of the replicates of each patient.  
...
```{r, include = F}
one_way_anova_BH <- raw_data_median$BH_FDR
one_way_anova_qval <- raw_data_median$Storey_Qvalues
qval_eta0 <- unname(fdrtool(raw_data_median$p_values, statistic = "pvalue", verbose = F, plot  = F)$param[,"eta0"])
all_peptide_hist <- hist(raw_data_median$p_values, breaks = 70, freq = F, 
                         xlab = "one-way ANOVA p-values", main = "p-values distribution for peptides")
```

```{r}
hist(raw_data_median$p_values, breaks = 70, freq = F, xlab = "one-way ANOVA p-values", 
     main = "p-values distribution for peptides")
